...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(filename)s:%(lineno)d - %(message)s', # Added filename/line
    handlers=[
        logging.FileHandler("asset_population.log", mode='w'), # Overwrite log each run
...
def delete_unwanted_assets_and_history(conn, active_symbols_set):
    ...
    try:
        ...
        tables_to_clear = {
            ...
            "order_status": "order_id IN (SELECT order_id FROM orders WHERE trade_id IN (SELECT trade_id FROM trades WHERE asset_id IN ({0})))".format(id_placeholders), # Complex dependency
